---
title: "Reproducibile Projects and Version Control"
subtitle: ""
author: "Itamar Caspi"
...
- A "reproducible" state-of-mind means putting yourself in the shoes of the consumers, rather than producers, of your code.
...
- If your R script starts with `setwd()` or `rm(list=ls())` than are [doing something wrong](https://www.tidyverse.org/articles/2017/12/workflow-vs-script/)!  

- Instead:
  
  1. Use RStudio's project environment.  
  
  2. Go to `Tools -> General Options -> General` and set the Save workspace to .RData on exit to **NEVER**.
...
R Markdown notebooks, by RStudio, are THE perfect tool for conducting reproducible research.

- The process of "knitting" an Rmd file starts with a clean slate.


An *.Rmd file is a "meta-document" that can be exported as a:
...
- To understand what `%>%` means, try associating it with the word "then".
...
I believe that there is a growing concencus in the #rstats community that we should [learn the tidyverse first](http://varianceexplained.org/r/teach-tidyverse/).

Nevertheless, note that the tidyverse is "utopian" in the sesnse that it strives toward perfection, and thus keeps changing. by contrast, base R is built to last.
...
We now breifly introduce two tidyvers' flagships: `dplyr` and `ggplot2`.

---

# `dplyr`: The grammer of data manipulation

- Tidymodels extends the tidyverse philosophy to modelling tasks.

- Key "verbs":

  - `filter()` - selects observations (rows).
  - `select()` - selects variables (columns).
  - `mutate()` - generate new variabels (columns).
...
- Git is seamlessly integrted within RStudio projects.

- GitHub Desktop is a great git client, that prefectly integrates GitHub. 
...
- Sorry. Think of "Dropbox" for code on steroids.
...
# Suggested workflow for a new R project
...
3. Click the "Fech origin" button.
...
[<i class="fa fa-github"></i> Source code](https://raw.githack.com/ml4econ/notes-spring2019/master/03-r-git/03-r-git.html)  
