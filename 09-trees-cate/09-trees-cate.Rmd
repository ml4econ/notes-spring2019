...
1. [Why Study Heterogenous Treatment Effects (HTE)?](#cate)

2. [Challenges in Estimating HTE](#split)

3. [Introducing Causal Trees](#trees)
...
- Ideally, we would like to know "personalized" treatment effects, i.e., the effect of treatment on an individual with $X_i=x$. 
...
- $N^{tr}$ - size of the  the estimation sample
...
_BENEFIT_: Valide inference (independent subsamples).
...
2. Honest causal tree (CT-A) which uses sample splitting:
...
Athey and Imbens (2016) consider two additional splitting rules:
  ...
  1. Fit based trees: split is based on the goodness-of-fit of the _outcome_, where fitting tkes into account $D_i$.
...
- In particular, the training sampel is split to training $\mathcal{S}^{tr,tr}$ and validation $\mathcal{S}^{tr,cv}$ sets and pruning the tree is constructed based on $\mathcal{S}^{tr,tr}$ and validated using $\mathcal{S}^{tr,cv}$.
...
__NOTE: __The causal forest algorithm is implemented in the [`grf`](https://github.com/grf-labs/grf) package (Tibshirani, Athey, and Wager).


---
# Notes on causal forests

- The user is requiered to select
...
# Assign each observation to a specifit leaf
...
Use `lm()` with interaction trems, e.g.,
...
df_all_lm %>% 
  filter(grepl(":W",term)) %>%  # keep only interaction terms
